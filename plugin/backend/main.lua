...
-- Open an external link in the user's browser (the loopback app validates the URL).
function OpenExternalUrl(a)
    return backend_request("POST", "/open-url", { url = a and a.url })
end

-- Ask the app to check for a plugin/app update now (the "Check for updates" button).
function CheckForUpdatesNow()
    return backend_request("POST", "/check-updates", {})
end
