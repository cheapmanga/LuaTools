    ...
    Addons.AddonSourceService addonSources)
{
    ...
    /// belongs to: the headless add pipeline and the loopback HTTP API, both driven from outside the app.
    /// </summary>
    /// <remarks>
    /// <para>Mirrors the dispatch in <c>DownloadViewModel.DownloadFromSourceAsync</c>. Without it those
    /// two callers sent every name to <see cref="CreateManifestJob"/>, i.e. through the lua.tools proxy -
    /// so picking a free source from the store-page popup or from an external client asked the proxy for
    /// a source it does not serve, and the user saw "download failed, check your connection".</para>
    ///
    ...
    /// apart. An ignore-case match here would quietly send paid traffic down the free path, and the
    /// reverse.</para>
    /// </remarks>
